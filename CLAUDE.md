...
Implemented per the MVP plan: `timespec`, `config`, `store`, `engine`, `server` with tools `events`/`status`/`apps`/`allow_app`/`deny_app` and the `openrhyme://events/recent` resource. `src/openrhyme_mcp/__init__.py` carries `__version__`. **The MVP design is approved: `docs/superpowers/specs/2026-09-01-mvp-mcp-server-design.md`** — read it before implementing; plans live in `docs/superpowers/plans/`. The engine's Part 1 (capture daemon + CLI) is complete; build against its MVP spec (`../OpenRhyme/docs/superpowers/specs/2026-09-01-mvp-capture-engine-design.md`, schema v1 in §7.1). Remaining work: dogfooding against a live engine capture, and future storage tiers.
